...
export class MemoryPage implements OnInit {
  ...
  public clickCheatMode() {
    ...
    // Si le clic n'est pas bloqué et que le cheat mode est desactivé...
    if (this.clickable && !this.cheatMode) {
      // ... alors activer le cheat mode et desactiver le clic
      this.cheatMode = true
  ...
  public revealCard(pos) {
    ...
    if (this.memoryCardList[pos].flipped === false && this.clickable) {
      ...
      this.clickable = false
      setTimeout(() => this.clickable = true, 2250);
      ...
      if (!this.firstClickedCard) {
        ...
        this.tryCount++
        // Si la deuxième carte cliquée est la paire de la première et que ce n'est pas deux fois la même carte à la meme position...
